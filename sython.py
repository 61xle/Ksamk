...
@sython.on(events.NewMessage(outgoing=False, pattern='عروف'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك عروف ابشر  @N_7_Z')
...
async def _(event):
      await event.reply("""السـورس يعمـل |عروف
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
